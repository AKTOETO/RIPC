...
void IPC::Client::call(Request &&req, std::function<void(IPC::Response &&)> &&callback)
{
...
void IPC::Client::handleJson(std::shared_ptr<Session> session, boost::json::value &&json)
{
	...
	{
		std::cerr << "[Client]: Обработчик не найден: " << req.m_uid << std::endl;
	}
}
