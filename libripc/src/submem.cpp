...
Memory::Memory(RipcContext &context)
    : m_context(context), m_addr(nullptr), m_is_mapped(false), m_max_size(-1)
// m_current_size(0)
{}
...
bool Memory::mmap(int first_id, int second_id) {
  ...
  //           std::dec << std::endl;
  LOG_INFO("%d Attempt to call mmap with offset 0x%x (packed 0x%x)", first_id,
           offset, packed_id);

  // запрос на отображение памяти
  char *addr = static_cast<char *>(::mmap(NULL, SHM_REGION_PAGE_SIZE,
                                          PROT_READ | PROT_WRITE, MAP_SHARED,
                                          m_context.getFd(), offset));
...
bool Memory::unmap() {
  ...
  if (munmap(m_addr, (m_max_size == -1 ? SHM_REGION_PAGE_SIZE : m_max_size)) !=
      0) {
    int err_code = errno;
...
char *Memory::end() const { return static_cast<char *>(m_addr) + m_max_size; }
...
size_t Memory::readUntil(size_t offset, char *buffer, size_t buffer_size,
                         char delim) {
  CHECK_MMAPED_R(-1);
  CHECK_ADDR_R(-1);
  CHECK_OFFSET_R(-1);
  ...
  if (read_len > 0) {
    void *result_ptr =
        memccpy(buffer, static_cast<char *>(m_addr) + offset, delim, read_len);
...
std::string Memory::read(size_t offset, size_t buffer_size) {
  ...
  if (read_len > 0) {
    ...
    auto read =
        memcpy(str_data, static_cast<char *>(m_addr) + offset, read_len);
  }
...
size_t Memory::write(size_t offset, const char *buffer, size_t buffer_size) {
  ...
  size_t write_len =
      std::min(buffer_size, available_space); // Реальное кол-во байт для записи

  // Копируем данные
  if (write_len > 0) {
    char *dest =
        static_cast<char *>(m_addr) + offset; // Указатель на место назначения
...
BufferView::BufferView(Memory &mem)
    : m_mem(mem), m_headers_finalized(false), m_memory_finalized(false),
      m_current_size(0) {
  if (!mem.m_is_mapped)
...
size_t BufferView::getCapacity() const { return m_mem.m_max_size; }

size_t BufferView::getCurrentSize() const { return m_current_size; }
